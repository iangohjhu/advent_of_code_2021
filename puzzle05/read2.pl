# day05 part 2 Now consider diagonal lines. At how many points do at least two lines overlap?
...
#	print "line xy1: $x1 $y1 , xy2: $x2 $y2\n";
	...
	{
		my $length = abs($x2 - $x1);
		my $dirx = ($x2 - $x1) / abs($x2 - $x1);
		my $diry = ($y2 - $y1) / abs($y2 - $y1);
		
		for ($i = 0; $i <= $length; $i++)
		{
			$array[$x1+ $dirx * $i][$y1+ $diry * $i] = $array[$x1 + $dirx * $i][$y1 + $diry * $i] + 1;
		}
	}
} 
